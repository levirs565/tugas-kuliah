...
namespace UtilitasTerminal
{
    ...
    /*
        ...
        Windows Command Prompt tidak menundkung ANSI escape
        Windows Terminal mendukung ANSI escape
    ...
    */
    ...
    namespace WinCon
    {
        ...
        bool aktifkanAnsi()
        {
            ...
            if (!SetConsoleMode(hStdOut, currentMode))
                return false;

            return true;
        }

        void nonaktifkanAnsi()
        {
            SetConsoleMode(GetStdHandle(STD_OUTPUT_HANDLE), previousMode);
        }
...
#ifdef _WIN32
    bool isUseAnsi = false;
#endif

    void aktifkan()
    {
#ifdef _WIN32
        isUseAnsi = WinCon::aktifkanAnsi();
#endif
    }
...
#ifdef _WIN32
        if (isUseAnsi)
            WinCon::nonaktifkanAnsi();
#endif

        nonaktifkanInputNavigasi();
    }

    string mesinSekarang()
    {
#ifdef _WIN32
        if (isUseAnsi)
            return "Windows Console API + ANSI";
        else
...
#ifdef _WIN32
        if (isUseAnsi)
            ANSI::tetapkanWarna(warna);
        else
            WinCon::tetapkanWarna(warna);
#else
        ANSI::tetapkanWarna(warna);
#endif
    }
    ...
    {
#ifdef _WIN32
        return WinCon::jumlahKolom();
#else
        return Unix::jumlahKolom();
#endif
    }

    bool inputNavigasiHidup = false;

    void aktifkanInputNavigasi()
    {
        if (inputNavigasiHidup)
            return;

#ifdef _WIN32

#else
        Unix::aktifkanInputNavigasi();
#endif

        inputNavigasiHidup = true;
    }

    Navigasi inputNavigasi()
    {
#ifdef _WIN32
        return WinCon::inputNavigasi();
#else
        return ANSI::inputNavigasi();
#endif
    }
...
#else
        Unix::nonaktifkanInputNavigasi();
#endif

        inputNavigasiHidup = false;
    }
...
#define HEADING_PADDING 2
...
#ifdef _WIN32
        if (isUseAnsi)
            ANSI::simpanPosisiCursor();
        else
            WinCon::simpanPosisiCursor();
#else
        ANSI::simpanPosisiCursor();
#endif
    }
    ...
    void kembalikanPosisiCursorDanHapus()
    {
        ...
        if (isUseAnsi)
			...
			WinCon::kembalikanPosisiCursorDanHapus();
#else
        ANSI::kembalikanPosisiCursorDanHapus();
#endif
    }
...
int main()
{
    UtilitasTerminal::aktifkan();
    atexit(UtilitasTerminal::nonaktifkan);

    cout << "Terminal menggunakan " << UtilitasTerminal::mesinSekarang() << endl;
